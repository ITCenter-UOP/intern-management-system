...
const createInternShipStartLetter = async ({ date, name, startdate, enddate, superviosrname }) => {
    try {
        ...
        const folderPath = path.join(process.cwd(), "uploads", "letters", "internshipstart"); // note plural 'uploads'
        ...
        return new Promise((resolve, reject) => {
            fileStream.on("finish", () => {
                // ✅ only return relative path for DB/frontend
                const relativePath = path.join("uploads", "letters", "internshipstart", filename);
